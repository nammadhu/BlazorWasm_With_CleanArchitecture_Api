...
namespace MyTown.Application.Features.Cards.Commands
    {
    public class UpdateTownCardCommandHandler(ITownCardRepository repository, IUnitOfWork unitOfWork
        , ITranslator translator, IMapper mapper) : IRequestHandler<UpdateTownCardCommand, BaseResult>
        {
        public async Task<BaseResult> Handle(UpdateTownCardCommand request, CancellationToken cancellationToken)
            {
